...
export const RunAttributionSchema = z.object({
  ...
  approval: z.object({
    ...
    /** Exact pre-release candidate reviewed by the readiness brains. */
    evidenceDigest: z.string().nullable().default(null),
    /** Null means no approval was attempted (for example an offline demo). */
  ...
  /** Exact generated bytes covered by the final verification receipt. */
  generatedFiles: z
    .array(
      z.object({
        path: z.string(),
    ...
    .default([]),
  /** Commit proven to contain the verification-receipt bytes, if delivered. */
  verifiedCommitSha: z
    .string()
    .regex(/^[a-f0-9]{40,64}$/)
    .nullable()
    .default(null),
  /** Sanitized, digest-bound record of commands actually attempted. */
  testReceipt: z
    .object({
    ...
    .nullable()
    .default(null),
  /** Recovery/VCS locator; this records a rollback path, not a claim it ran. */
  rollback: z
    .object({
      ...
      trunkAdvancePath: z.enum(["pr-gate", "direct-fast-forward"]).nullable(),
    })
    .nullable()
    .default(null),
