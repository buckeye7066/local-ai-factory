...
function openingTagSuppressesText(tag: string, rawTag: string): boolean {
  ...
  // HTML character references are resolved while the browser parses the
  // attribute, before the resulting value is parsed as CSS. Preserve that
  // order here so encoded punctuation cannot disguise a hidden declaration.
  const normalizedStyle = decodeCssEscapes(
    decodeHtmlEntities(rawStyle).replace(/\/\*[\s\S]*?\*\//g, ""),
  );
