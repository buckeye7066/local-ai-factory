  ...
  it("marks the run failed (timed out) instead of hanging past the configured deadline", async () => {
    ...
    const run = startRun({
      ...
      options: {
        ...
        // Leave enough scheduling headroom for a saturated full-suite worker
        // to reach the provider call; the call itself still never resolves and
        // must be cut off by this sub-second run deadline.
        timeoutMs: 750,
      },
      config,
      secrets,
    ...
    // Bounded by the configured 750ms deadline plus scheduling slack — nowhere
