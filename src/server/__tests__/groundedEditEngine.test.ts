...
import {
  canReplaceFullyShownGeneratedTest,
...
describe("repair accounting and scope are mechanical", () => {
  it("lets Test Writer replace only a fully shown test generated by this run", () => {
    const eligible = {
      path: "tests/taskline.test.ts",
      existedBefore: true,
    ...
    expect(
      canReplaceFullyShownGeneratedTest({
        ...eligible,
        priorStatus: "modified",
      }),
    ).toBe(false);
    expect(
      canReplaceFullyShownGeneratedTest({
        ...eligible,
        suppliedInFull: false,
      }),
    ).toBe(false);
    expect(
      canReplaceFullyShownGeneratedTest({
