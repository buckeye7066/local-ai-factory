...
export interface ReleaseInput {
  ...
  /** Resume the saved PR even after GitHub deleted its merged branch. */
...
export interface ReleaseResult {
  /** External conditions may be retried; failed evidence must not be auto-overridden. */
  retryable?: boolean;
  released: boolean;
  state: ReleaseState;
  prUrl: string | null;
...
type HostCheck = { state: string; name: string };
function parseHostChecks(raw: string): HostCheck[] | null {
  try {
...
export async function releaseRun(input: ReleaseInput): Promise<ReleaseResult> {
  ...
  const inspectExisting = async (
    reference: string,
  ): Promise<{ url: string | null; result?: ReleaseResult }> => {
    ...
    if (!succeeded(view))
      return {
        url: null,
        result: held(
          `could not inspect the saved PR: ${failureText(view)}`,
          null,
          true,
        ),
      };
    const [url, state, head, mergedSha] = view.stdout.trim().split("	");
    if (!url || head !== input.verifiedCommitSha)
      return {
        url: url || null,
        result: held(
          "the existing PR head does not match the exact verified delivered commit",
          url || null,
        ),
      };
    ...
    if (state !== "OPEN")
      return {
        url,
        result: held(
          `saved PR state is ${state || "unknown"} without a confirmed merge; not claiming release`,
          url,
        ),
      };
    return { url };
  ...
  // Reconcile before creating anything. After auto-merge, `pr create` can say
  // "no commits between" rather than "already exists", and the branch may be gone.
  let prUrl: string | null = null;
  if (input.existingPrUrl) {
    const match =
      /^https:\/\/github\.com\/([^/]+)\/([^/]+)\/pull\/([1-9]\d*)\/?$/i.exec(
        input.existingPrUrl,
      );
    if (!match || `${match[1]}/${match[2]}`.toLowerCase() !== slug.toLowerCase()) {
      return held("saved PR does not belong to the destination repository");
    }
    const existing = await inspectExisting(input.existingPrUrl);
    if (existing.result) return existing.result;
    prUrl = existing.url;
  } else {
    // Omit --base so GitHub resolves the destination's real default branch.
    ...
    if (succeeded(create)) prUrl = create.stdout.trim().split(/\s+/).pop() ?? null;
    else if (/already exists|no commits between/i.test(create.stderr + create.stdout)) {
      const existing = await inspectExisting(input.branch);
      if (existing.result) return existing.result;
      prUrl = existing.url;
    }
    if (!prUrl)
      return held(`could not open the PR: ${failureText(create)}`, null, true);
  }
  if (!prUrl) return held("the saved PR could not be resolved", null, true);
  ...
  for (;;) {
    ...
    if (checks.spawnError || ![0, 1, 8].includes(checks.code ?? -1)) {
      return held(`host checks could not be read: ${failureText(checks)}`, prUrl, true);
    }
    if (/no checks reported/i.test(raw)) {
      if (observedHostChecks)
        return held(
          "previously observed host checks disappeared; not treating this as a repository without CI",
          prUrl,
          true,
        );
      absentObservations++;
    ...
    let states = parseHostChecks(checks.stdout);
    if (!states)
      return held(
        "host checks returned invalid evidence; not arming a merge",
        prUrl,
        true,
      );
    if (!states.length)
      return held(
        "host checks returned no usable evidence; waiting for GitHub registration",
        prUrl,
        true,
      );
    observedHostChecks = true;
    if (states.some((check) => /^(SKIPPED|NEUTRAL)$/i.test(check.state))) {
      const requiredRead = await run(
        ["pr", "checks", prUrl, "--required", "--json", "state,name"],
        process.cwd(),
        60_000,
      );
      let required = parseHostChecks(requiredRead.stdout);
      if (
        !requiredRead.spawnError &&
        [0, 1].includes(requiredRead.code ?? -1) &&
        /no required checks reported/i.test(requiredRead.stdout + requiredRead.stderr)
      )
        required = [];
      if (
        requiredRead.spawnError ||
        ![0, 1, 8].includes(requiredRead.code ?? -1) ||
        !required
      ) {
        return held(
          "could not determine required checks; optional checks were not assumed",
          prUrl,
          true,
        );
      }
      // Required check names are conservative identities: ambiguity stays required.
      const requiredNames = new Set(required.map((check) => check.name));
      states = states
        .filter(
          (check) =>
            !requiredNames.has(check.name) && !/^(SKIPPED|NEUTRAL)$/i.test(check.state),
        )
        .concat(required);
      if (!states.length)
        return held(
          "host checks were only optional skipped/neutral results; no successful executed host check was observed",
          prUrl,
        );
    }
