...
const timeoutMs = Number(
  process.env.PURPOSE_FOUNDRY_SMOKE_TIMEOUT_MS || 21000000,
);
...
if (current.status !== "completed") {
  const details = current.stations
    ...
    .join("; ");
  throw new Error(
    `Purpose Foundry ended ${current.status}: ${details || "no terminal detail"}`,
  );
}

const readiness = await request(
  `/api/foundry/projects/${project.id}/readiness`,
);
